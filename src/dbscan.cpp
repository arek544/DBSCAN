...
void dbscan(const xarray<float> &X_original, float epsilon, int minPts, std::ofstream &outfile)
{
    ...
    function<void(int)> search = [&](int current_index) -> void
    {
        ...
        outfile << std::setprecision(16) << current_time() << ",Eps_time," << current_index << "," << elapsed.count() << ',' << endl;
        outfile << std::setprecision(16) << current_time() << ",|Eps_neighbors|," << current_index << "," << (int)neighbor_indices.size() << ',' << endl;
        outfile << std::setprecision(16) << current_time() << ",Eps_neighbor_id," << current_index << ",,";
